...
export default function RepertoirePage() {
  ...
  useEffect(() => {
    if (selectedHotelId) fetchEntries();
  }, [selectedHotelId]);
         ...
         <div className="p-4 border-b border-slate-100 space-y-4">
            <div>
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={handleCreate} className="p-2 bg-[var(--brand-bg)] text-[var(--brand)] rounded-lg hover:bg-[var(--brand-bg)] transition" title="Nouveau contact">
                ...
                <div 
                    ...
                    className={`
                        ...
                        ${selectedEntry?.id === entry.id 
                            ? 'bg-[var(--brand-bg)] border-[var(--brand)] text-indigo-900 shadow-sm' 
                            : 'hover:bg-slate-50 text-slate-700 hover:border-slate-100'
                        }
                    `}
                >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 font-bold text-sm ${selectedEntry?.id === entry.id ? 'bg-[var(--brand-bg)] text-[var(--brand)]' : 'bg-slate-100 text-slate-500'}`}>
          ...
          {(selectedEntry || isEditing) ? (
              <div className="w-full max-w-xl bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-300">
                          ...
                          <div>
                              {isEditing ? (
                                  <div className="space-y-2">
                                      <input 
                                          className="text-xl font-bold text-slate-900 placeholder:text-slate-300 outline-none bg-transparent w-full border-b border-slate-300 focus:border-[var(--brand)] pb-1 transition-colors"
                                      ...
                                      />
                                      <input 
                                          className="text-sm font-medium text-slate-600 placeholder:text-slate-300 outline-none bg-transparent w-full border-b border-slate-300 focus:border-[var(--brand)] pb-1 transition-colors"
                      ...
                      <div className="flex gap-2">
                          {isEditing ? (
                              <>
                                  ...
                                  <button onClick={saveEntry} className="p-2 btn-brand text-white rounded-lg hover:brightness-110 shadow-md transition transform active:scale-95">
                              ...
                              <>
                                  ...
                                  <button onClick={() => { setForm({ qui_quoi: selectedEntry!.qui_quoi, contact: selectedEntry!.contact, commentaire: selectedEntry!.commentaire || "" }); setIsEditing(true); }} className="p-2 text-[var(--brand)] hover:bg-[var(--brand-bg)] rounded-lg transition">
                  ...
                  <div className="p-8">
                      ...
                      {isEditing ? (
                          <textarea 
                              className="w-full h-32 resize-none outline-none text-sm text-slate-700 leading-relaxed placeholder:text-slate-300 bg-slate-50 p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[var(--brand)] transition-all"
                  ...
                  {!isEditing && selectedEntry && (
                      <div className="bg-slate-50 p-4 border-t border-slate-100 flex gap-4 justify-center">
                           {selectedEntry.contact.includes('@') && (
                               <a href={`mailto:${selectedEntry.contact}`} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-bold text-slate-700 hover:border-[var(--brand)] hover:text-[var(--brand)] transition shadow-sm">
